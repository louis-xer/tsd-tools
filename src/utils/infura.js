...
web3 = new Web3(
  new Web3.providers.HttpProvider('https://mainnet.infura.io/v3/5b61867f91844cb7aa5f20a0d9068e84'),
)
...
export const getCouponEpochs = async (dao, account) => {
  ...
  const events = bought
    .map(e => ({ epoch: e.returnValues.epoch, amount: e.returnValues.couponAmount }))
...
export const getCost = async amount => {
  ...
  const [inputAmount, _] = await exchange.methods
    .getAmountsIn(new BigNumber(amount).toFixed(), [USDC.addr, TSD.addr])
    .call()
  return inputAmount
}
...
export const getProceeds = async amount => {
  ...
  const [_, outputAmount] = await exchange.methods
    .getAmountsOut(new BigNumber(amount).toFixed(), [TSD.addr, USDC.addr])
    .call()
  return outputAmount
}
