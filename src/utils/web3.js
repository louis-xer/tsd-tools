...
export const buyTSD = async (buyAmount, maxInputAmount) => {
  ...
  await router.methods
    .swapTokensForExactTokens(buyAmount, maxInputAmount, [USDC.addr, TSD.addr], account, deadline)
    .send({ from: account })
    .on('transactionHash', hash => {
      notify.hash(hash)
    })
...
export const sellTSD = async (sellAmount, minOutputAmount) => {
  ...
  await router.methods
    .swapExactTokensForTokens(sellAmount, minOutputAmount, [TSD.addr, USDC.addr], account, deadline)
    .send({ from: account })
    .on('transactionHash', hash => {
      notify.hash(hash)
    })
