...
import { TSD, UNI, BUSD } from '../constants-bsc/tokens'
import { POOL_EXIT_LOCKUP_EPOCHS } from '../constants-bsc/values'

const dollarAbi = require('../constants-bsc/abi/Dollar.json')
...
const pancakePairAbi = require('../constants-bsc/abi/PancakePair.json')
...
export const loadFluidStatusDao = async (dao, account) => {
  const daoContract = new web3.eth.Contract(daoAbi, dao)
  const bondP = await daoContract.getPastEvents('Bond', {
    filter: { account },
    fromBlock: 0,
  })
  const unbondP = await daoContract.getPastEvents('Unbond', {
    filter: { account },
    fromBlock: 0,
  })
...
export const getCost = async amount => {
  const exchange = new web3.eth.Contract(pancakeRouterAbi, PancakeRouter02)
  // eslint-disable-next-line no-unused-vars
  const [inputAmount, _] = await exchange.methods
    .getAmountsIn(new BigNumber(amount).toFixed(), [BUSD.addr, TSD.addr])
    .call()
  return inputAmount
}
...
export const getProceeds = async amount => {
  ...
  const [_, outputAmount] = await exchange.methods
    .getAmountsOut(new BigNumber(amount).toFixed(), [TSD.addr, BUSD.addr])
    .call()
  return outputAmount
}

export const getReserves = async () => {
  const exchange = new web3.eth.Contract(pancakePairAbi, UNI.addr)
...
export const getTotalSupplyUni = async () => {
  const tokenContract = new web3.eth.Contract(pancakePairAbi, UNI.addr)
  return tokenContract.methods.totalSupply().call()
}

export const getPrice0CumulativeLast = async () => {
  const price0 = new web3.eth.Contract(pancakePairAbi, UNI.addr)
...
export const loadFluidStatusPool = async (pool, account) => {
  const poolContract = new web3.eth.Contract(poolAbi, pool)
  const bondP = await poolContract.getPastEvents('Bond', {
    filter: { account },
    fromBlock: 0,
  })
  const unbondP = await poolContract.getPastEvents('Unbond', {
    filter: { account },
    fromBlock: 0,
  })
